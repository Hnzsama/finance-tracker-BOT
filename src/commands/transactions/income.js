...
export default {
    ...
    execute: async (sock, message, text) => {
        ...
        try {
            ...
            // Expected format: $masuk <amount> <category> <desc>
            // Example: $masuk 100000 Gaji Bulan ini
            const args = text.replace("$inc", "").trim().split(" ");

            if (args.length < 2) {
                return sock.sendMessage(from, { text: "⚠️ Format salah. Contoh: $inc 5000000 Gaji Bonus Tahunan" });
            }
            ...
            if (!category) {
                return sock.sendMessage(from, { text: `⚠️ Kategori '${categoryName}' tidak ditemukan. Cek $list-cat` });
            }
...
╰ _Cek history: $list-trx_`
            ...
            await sock.sendMessage(from, { text: "❌ Terjadi kesalahan saat mencatat pemasukan." });
        }
    },
};
