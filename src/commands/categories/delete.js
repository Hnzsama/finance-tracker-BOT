...
export default {
    ...
    execute: async (sock, message, text) => {
        ...
        try {
            ...
            if (!rawInput) {
                return sock.sendMessage(chatId, { text: "⚠️ Format salah. Contoh: $del-cat Makanan, Gaji" });
            }
...
╰ _Cek list: $list-cat_`
                ...
                await sock.sendMessage(chatId, { text: "⚠️ Tidak ada kategori yang ditemukan untuk dihapus." });
            }
            ...
            if (error.code === 'P2003') {
                await sock.sendMessage(chatId, { text: "⚠️ Gagal menghapus: Kategori ini sedang digunakan dalam transaksi." });
            } else {
                await sock.sendMessage(chatId, { text: "❌ Terjadi kesalahan saat menghapus kategori." });
            }
        }
    },
};
